...
subplot(2,3,3); plot(x,c);xlabel("Time,s"); ylabel("угол соб.вращени€, rad");grid;
...
figure(2); plot(x,q_norm); xlabel("Time,s"); ylabel("quaternion norm"); grid;

figure(3);
plot3(y(8,:),y(9,:),y(10,:)); grid;

figure(4);
subplot(2,3,1); plot(x,B_b(1,:));xlabel("Time,s"); ylabel("B_x, “л");grid;
subplot(2,3,2); plot(x,B_b(2,:));xlabel("Time,s"); ylabel("B_y, “л");grid;
subplot(2,3,3); plot(x,B_b(3,:));xlabel("Time,s"); ylabel("B_z, “л");grid;

subplot(2,3,4); plot(x, w(1,:)); xlabel("Time,s"); ylabel("w_x_g_y_r_o, rad/s"); grid;
subplot(2,3,5); plot(x, w(2,:)); xlabel("Time,s"); ylabel("w_y_g_y_r_o, rad/s"); grid;
subplot(2,3,6); plot(x, w(3,:)); xlabel("Time,s"); ylabel("w_z_g_y_r_o, rad/s"); grid;
